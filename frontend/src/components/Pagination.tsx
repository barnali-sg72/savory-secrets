...
export default function Pagination(props: Props) {
    ...
    return (
        <section className="d-flex flex-column justify-content-center">
            ...
            <div className="page-buttons btn-group justify-content-center">
                <button className="btn btn-primary" disabled={prevDisabled}
                ...
                { pages && pages > 0 ? 
                    range(displayStart, displayEnd+1).map((v:number, k:number) => (
                        <button className={selected === v ? "btn btn-primary selected": "btn btn-primary"} 
                            key={k} onClick={() => handleButtonClick(v)}>{v}</button>
                    ))                  
                    : <></> 
                }
                { displayEnd < pages ? 
