...
import { getLoggedUser, PageLayoutType } from "../App";
...
export default function RecipePage(props: Props) {
  ...
  useEffect(() => {
    ...
    props.menuData.searchCriteria,
    props.menuData.mealType,
    props.menuData.ingredientType,
    props.menuData.isUserRecipes,
  ]);
  ...
  const getRecipeList = () => {
    ...
    axios
      ...
      .then((response) => {
        setRecipeList(
          response.data.data[0].filter((d: Recipe) => {
            return (
              props.menuData.isUserRecipes ||
