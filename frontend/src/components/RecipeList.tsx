...
import { OutletContextType, RecipeContext, RecipeContextType } from "./RecipePage";
...
export default function RecipeList() {
    ...
    const mediaMatches: boolean = useMediaQuery("only screen and (min-width: 768px)");
    ...
    const handleRecipeClick = (event: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    ...
    return (
        <section className="d-flex flex-column px-4 pb-4">
            <div className="d-flex-inline w-100 form-header mb-3 mt-4 p-1 rounded-2">
                <div className="mt-2 ms-2">
                    <h2><b>{ recipeContext.title.toUpperCase()}</b></h2>
                </div>
                {/*<div className="float-end">
                    <button className="btn btn-link" onClick={(e) => handleAddClick(e)}>
                        <PlusSquare/>
                    </button>
                </div>*/}
            </div>  
            ...
            :<section className="d-flex justify-content-center">
                <div className="d-flex align-content-start flex-wrap col-scroll gap-4 ms-3 pt-4 ps-3">
                    {outletData.recipeList.map((rec) => (
                        <article className="card position-relative" onClick={() => navigate("/recipes/display/"+ rec.id)}>
                            ...
                            <div className="card-body"> 
                                ...
                                <h6 className="card-author mb-3"><b>Author:</b> {rec.author.firstname + " " + rec.author.lastname}</h6>
                                <h6 className="card-title text-wrap">{rec.title}</h6>           
                                {<p className="card-text" dangerouslySetInnerHTML={{ __html:(rec.description || '') }}></p>}
                            </div>
                        </article>
