...
function App() {  
    ...
    const saveUser = async (user: User|null, mode:string):Promise<Result> => {
        ...
        if (mode == "signup") {
            ...
            response = await axios.put("http://localhost:8000/user/"+user?.id, user);
        }
        ...
        if (response.status == 200){
            ...
            if (response.data.code == 409) {
                ...
                isError = false;
                if (mode == "signup") {
                    msg = "User "+user?.username + " added successfully. Please go to the login page";
                } else {
                    if (data != undefined) {
            ...
            isError = true;     
            msg = "User "+user?.username + " could not be saved successfully. Please try later";
        }
  ...
  return (
    <UserContext.Provider value={UserContextValue}>
        <div className='App d-flex flex-column'>
            ...
            <Main searchCriteria={searchCriteria} mealType={mealType} ingredientType={ingredientType}
                isUserRecipes={isUserRecipes} searchValue={updateSearch}/>
            <Footer/>            
        </div>
